...
export interface UseChatOptions {
  ...
  /** Called after each successful chat response (e.g. to refetch tasks when chatbot used tools). */
  onResponse?: (response: ChatResponse) => void;
  /** When false, do not load or refetch the conversation list (e.g. for icon/widget chat). Default true. */
...
export interface UseChatResult {
  ...
  retry: () => Promise<void>;
  clearError: () => void;
  /** Start a new conversation: clears current conversation and messages in the UI. */
  startNewConversation: () => void;
  /** Switch to a conversation (load its messages). */
  setCurrentConversationId: (id: number | null) => void;
  refetchConversations: () => Promise<void>;
  /** Delete a conversation and refresh list; if current was deleted, clear or switch to another. */
...
export function useChat({ userId, onResponse, loadConversationList = true }: UseChatOptions): UseChatResult {
  ...
  // Load conversations list (only when loadConversationList is true, e.g. full chat page)
  ...
  // Load messages when conversationId is set (from DB: messages table)
  useEffect(() => {
